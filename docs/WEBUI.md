...
# diary only, no looks:
lucy-daemon --ui-only --engine local --log-dir logs
```
...
Not IP packets, TLS records, or radio frames. A weather.com open can be dozens of fetches. Harvest is host-only. [HARVEST.md](HARVEST.md).
